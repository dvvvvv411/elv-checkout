...
import { fetchOrderConfirmation, OrderFetchError } from "@/lib/order-api";
...
type LoadState =
  | { status: "loading" }
  | { status: "success"; data: OrderConfirmationData }
  | { status: "error"; error: OrderFetchError };
...
function ConfirmationPage() {
  ...
  useEffect(() => {
    ...
    fetchOrderConfirmation(orderNumber)
      .then((data) => {
  ...
  useEffect(() => {
    if (state.status === "success" && state.data.session.branding.company_name) {
      document.title = `Bestellbestätigung — ${state.data.session.branding.company_name}`;
    }
  }, [state]);
  ...
  return (
    <CheckoutSessionProvider session={state.data.session} token={null}>
      <ConfirmationContent order={state.data} />
    ...
    <div className="flex min-h-screen items-center justify-center bg-background px-4">
      <div className="animate-fade-in flex flex-col items-center gap-5">
        ...
        <div className="text-center">
          <p className="text-sm font-medium text-foreground">Bestellung wird geladen</p>
...
function ConfirmationErrorState({
  ...
  const isNotFound = error.kind === "not_found" || error.kind === "invalid_input";
    ...
    <div className="min-h-screen bg-background">
      <main className="mx-auto flex max-w-md flex-col items-center px-4 py-20 text-center">
        ...
        <h1 className="text-xl font-semibold text-foreground">
          {isNotFound ? "Bestellung nicht gefunden" : "Bestellung konnte nicht geladen werden"}
        </h1>
        <p className="mt-2 text-sm text-muted-foreground">
          {isNotFound
            ? "Die angegebene Bestellnummer existiert nicht. Bitte prüfe den Link aus deiner Bestätigungsmail."
            : error.message}
        </p>
        <div className="mt-6 flex flex-col items-center gap-3 sm:flex-row">
          {!isNotFound && (
            <Button onClick={onRetry} className="gap-2 bg-gradient-primary shadow-elegant">
